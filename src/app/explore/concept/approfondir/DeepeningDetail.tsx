...
export function DeepeningDetail() {
  ...
  return (
    <Screen>
      {/*
       * La colonne d'un écran de lecture, et les trois mesures qu'elle porte.
       *
       * Cet écran avait sa largeur propre — `max-w-[32rem]`, puis `--container-read` au
       * premier seuil —, et elle était juste tant qu'un écran de lecture n'était qu'une
       * colonne. Elle ne l'est plus dès qu'un en-tête et un rail l'accompagnent : le
       * titre gagne à la largeur, le corps n'y gagne rien, et les cinq notices
       * bibliographiques attendaient tout en bas de huit écrans de défilement.
       *
       * Ce qui ne change pas est ce qui comptait : le corps reste à 496 px, parce que
       * c'est la ligne qui le décide. Les deux réglages propres au téléphone — 32 rem de
       * colonne, 20 px de gouttière — sont maintenant dans `.column-serif`, avec leur
       * raison ; ils avaient été pris ici quand cet écran était le seul concerné.
       *
       * `enter-rise` porte l'arrivée du texte, et il ne la porte que parce que l'attente
       * vient de finir : à l'intérieur d'un même `Screen`, le remplacement de l'écran
       * d'attente par l'article n'est pas une navigation et ne déclenche aucune transition
       * de vue. Sans lui, mille cinq cents mots apparaîtraient d'une image à l'autre au
       * terme d'une cérémonie de trois secondes, ce qui est le seul endroit où elle
       * pouvait encore se rater. En mouvement réduit, la classe ne joue pas et le texte
       * est simplement là.
       */}
      <article className="enter-rise column column-editorial column-serif pt-10 pb-12">
        ...
        <div className="reading-shell">
          <div className="reading-layout reading-long">
            {/*
             * L'en-tête éditorial : ce qui nomme le texte et ce qui le situe. `header` est
             * ici sans danger — il est dans un `article`, où il désigne l'en-tête de
             * l'article et non celui du site.
             */}
            ...
            {/*
             * Le corps, borné par la ligne. L'accroche l'ouvre et n'appartient pas à
             * l'en-tête : c'est la question à laquelle le texte répond, elle se lit avec
             * lui et à sa mesure.
             */}
            <div className="reading-body">
               ...
               *
               * Il reste donc dans le corps et ne monte pas au rail, alors qu'il en a la
               * nature : au rail, il serait visible dès la première ligne, c'est-à-dire
               * exactement l'avertissement en tête qu'on s'interdit.
               *
            ...
            {/*
             * Les sources, à côté du texte plutôt que huit écrans plus bas.
             *
             * C'est le seul écran où le rail change réellement quelque chose : on consulte
             * une référence **pendant** qu'on lit le passage qui s'y appuie, et une liste
             * atteinte après mille cinq cents mots ne sert plus qu'à celui qui la
             * cherchait déjà. Le rail suit donc le défilement — voir `.reading-rail`.
             *
             * Sur un téléphone, rien ne change : le bloc reprend sa place sous le texte,
             * dans le même ordre, avec la même liste. C'est la grille qui le déplace, pas
             * un second rendu.
             */}
