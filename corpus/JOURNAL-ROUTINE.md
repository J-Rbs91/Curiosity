...
- branche      : `claude/zen-johnson-jbbigs` (imposée par la session, pas `main`). **Le travail du passage 07 est fusionné dans `main`** (PR [#83](https://github.com/J-Rbs91/Curiosity/pull/83), commit de merge `f72ddc1`), et cette branche partait de `main` à jour, au commit `1929441` : rien n'a été refait. Pull request vers `main` ouverte à la clôture. Tant qu'elle n'est pas fusionnée, **la nuit suivante reprend depuis cette branche**, et non depuis `main`.
