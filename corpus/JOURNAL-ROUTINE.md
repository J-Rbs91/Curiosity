...
- branche      : `claude/zen-johnson-u4gu6o` (imposée par la session, pas `main`). **Le travail du passage 07 est fusionné dans `main`** (PR [#84](https://github.com/J-Rbs91/Curiosity/pull/84), commit de merge `bcf27c5`), et cette branche partait de `main` à jour, au commit `d281b23`, à zéro commit d'écart : rien n'a été refait. Pull request [#87](https://github.com/J-Rbs91/Curiosity/pull/87) vers `main`, **ouverte et non fusionnée à la clôture**. Tant qu'elle n'est pas fusionnée, **la nuit suivante reprend depuis cette branche**, et non depuis `main`, sinon elle refera ce qui est déjà fait.
